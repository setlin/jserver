...
struct net_pool
{
    ...
    // server id, save the server's index according to prot100 and unique code
    int serverid;

    map<int, int>* onlineconnidtocharid;
    map<int, int>* onlinecharidtoconnid;
};
...
void net_delete(struct net_pool** pnp)
{
    ...
    delete np->onlineconnidtocharid;
    delete np->onlinecharidtoconnid;
...
static void net_force_close(struct net_pool* np, struct net_socket* ns)
{
    ...
    net_remove_onlinechar(np, ns->id);
}
...
void net_set_serverid(struct net_pool* np, int serverid)
{
    np->serverid = serverid;
}

int net_get_serverid(struct net_pool* np)
{
    return np->serverid;
}

void net_set_onlinechar(struct net_pool* np, int connid, int charid)
{
    np->onlinecharidtoconnid->insert(pair<int, int>(charid, connid));
    np->onlineconnidtocharid->insert(pair<int, int>(connid, charid));
}

int net_get_onlinecharidbyconnid(struct net_pool* np, int connid)
{
    ONLINEINFOITER iter = np->onlineconnidtocharid->find(connid);

    if(iter == np->onlineconnidtocharid->end())
    {
        log_debug("connid %d is offline\n", connid);
        return -1;
    }
...
int net_get_onlineconnidbycharid(struct net_pool* np, int charid)
{
    ONLINEINFOITER iter = np->onlinecharidtoconnid->find(charid);

    if(iter == np->onlinecharidtoconnid->end())
    {
        log_debug("charid %d is offline\n");
        return -1;
    }
...
void net_remove_onlinechar(struct net_pool* np, int connid)
{
    // onlineconnidtocharid
    ONLINEINFOITER iter = np->onlineconnidtocharid->find(connid);
    int charid = -1;
    ...
    charid = iter->second;
    np->onlineconnidtocharid->erase(iter);

    // onlinecharidtoconnid
    iter = np->onlinecharidtoconnid->find(charid);

    if(iter == np->onlinecharidtoconnid->end())
    {
        log_debug("charid %d does not exist\n");
        return;
    }

    np->onlinecharidtoconnid->erase(iter);
}
