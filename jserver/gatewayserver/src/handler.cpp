...
static void ontranspond_sendprot_toserver_setconnid(struct net_pool* np, uint8_t* protdata, int connid)
{
    log_debug("ontranspond_sendprot_toserver_setconnid\n");
    protdata[0] = (connid >> 24) & 0xFF;
    protdata[1] = (connid >> 16) & 0xFF;
    protdata[2] = (connid >> 8) & 0xFF;
    protdata[3] = (connid ) & 0xFF;

    int charid = net_get_onlinecharidbyconnid(np, connid);
    ...
    if(header > 0)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    log_error("TODO[[the client is offline]]");
                }
            }
        }
        free(protdata);
